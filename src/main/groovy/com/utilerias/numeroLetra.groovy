...
public class NumeroLetra {
    ...
    private String miles(int numero){
        ...
        if (numero >= 2000 && numero <10000){
            flag=1;
            num_letram = unidad(numero/1000 as int).concat(" mil ").concat(centena(numero%1000));
        }
        if (numero < 1000)
    ...
    private String decmiles(int numero){
        ...
        if (numero > 10000 && numero <20000){
            flag=1;
            num_letradm = decena(numero/1000 as int).concat("mil ").concat(centena(numero%1000));
        }
        if (numero >= 20000 && numero <100000){
            flag=1;
            num_letradm = decena(numero/1000 as int).concat(" mil ").concat(miles(numero%1000));
        }
    ...
    private String cienmiles(int numero){
        ...
        if (numero >= 100000 && numero <1000000){
            flag=1;
            num_letracm = centena(numero/1000 as int).concat(" mil ").concat(centena(numero%1000));
        }
        if (numero < 100000)
    ...
    private String millon(int numero){
        ...
        if (numero >= 2000000 && numero <10000000){
            flag=1;
            num_letramm = unidad(numero/1000000 as int).concat(" millones ").concat(cienmiles(numero%1000000));
        }
        if (numero < 1000000)
    ...
    private String decmillon(int numero){
        ...
        if (numero > 10000000 && numero <20000000){
            flag=1;
            num_letradmm = decena(numero/1000000 as int).concat("millones ").concat(cienmiles(numero%1000000));
        }
        if (numero >= 20000000 && numero <100000000){
            flag=1;
            num_letradmm = decena(numero/1000000 as int).concat(" milllones ").concat(millon(numero%1000000));
        }
    ...
    public String convertirLetras(String numero){
        ...
        for(int i=0;i<aux.length;i++){
            ...
            try{
                Integer e= Integer.parseInt(aux[i]);
                numero=numero.replace(aux[i], decmillon(e));
    ...
    public String convierteMontoALetras (double entrada){
        def entradaMoneda = java.text.NumberFormat.getCurrencyInstance(Locale.US).format(entrada as Number).replace(',','').replace('$','').replace('.','-')
        def arrx = (entradaMoneda).toString().split('-')
        def enteros=entrada as int
        def decimales=arrx[1] as int
        def res = convertirLetras(enteros).toUpperCase()
        if(enteros==0){
            res="CERO"
        }
        ...
        if(decimales>0){
            ...
            decimales="("+decimales+"/100) M.N."
        }else{
            decimales='00/100 M.N.'
        }
        return res+' PESOS '+decimales
    }
}
